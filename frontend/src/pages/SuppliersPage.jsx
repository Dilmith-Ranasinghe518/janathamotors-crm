...
export function SuppliersPage() {
  ...
  function submitPurchase(e) {
    ...
    if (!purchaseSupplierId || items.length === 0) {
      setError('Choose a supplier and add at least one item.')
      return
    }
  ...
  return (
    <div>
        ...
        actions={
          can('manage_suppliers') && (
            <>
              ...
              <Button onClick={() => setPurchaseOpen(true)}>+ New purchase</Button>
            </>
          )
        }
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card className="overflow-x-auto">
          <p className="border-b border-[var(--line)] px-4 py-3 text-sm font-semibold">Recent purchases</p>
          {purchases.length === 0 ? (
              ...
              <tbody>
                {purchases.map((p) => (
                  <tr key={p.id} className="border-b border-[var(--line)] last:border-0">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">{Number(p.total).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {supplierFormOpen && (
        <Modal title="New supplier" onClose={() => setSupplierFormOpen(false)}>
          ...
          >
            <Input label="Name" required value={supplierForm.name} onChange={(e) => setSupplierForm({ ...supplierForm, name: e.target.value })} />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={saveSupplierMutation.isPending}>Save</Button>
            </div>
          </form>
        </Modal>
      )}
        ...
        <Modal title="New purchase (stock in)" onClose={() => setPurchaseOpen(false)} wide>
          <form onSubmit={submitPurchase} className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input label="Date" type="date" required value={purchaseDate} onChange={(e) => setPurchaseDate(e.target.value)} />
            </div>
            ...
            <div className="flex flex-col gap-3">
              {lines.map((line, index) => (
                <div key={index} className="grid grid-cols-12 gap-2">
                  ...
                  <Input className="col-span-3" type="number" min="1" placeholder="Qty" value={line.quantity} onChange={(e) => updateLine(index, { quantity: e.target.value })} />
                  <Input className="col-span-3" type="number" step="0.01" placeholder="Unit cost" value={line.unit_cost} onChange={(e) => updateLine(index, { unit_cost: e.target.value })} />
                </div>
              ))}
            </div>
            <Button type="button" variant="secondary" onClick={() => setLines((prev) => [...prev, { ...blankLine }])}>+ Add line</Button>

            {error && <p className="text-sm text-[var(--critical)]">{error}</p>}
            <div className="flex justify-end gap-2 border-t border-[var(--line)] pt-4">
              <Button type="button" variant="secondary" onClick={() => setPurchaseOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={createPurchaseMutation.isPending}>Save purchase</Button>
            </div>
          </form>
        </Modal>
      )}
